...
void requestAttribute(uint8 * data) {
	...
	status = zcl_SendCommand( ENDPOINT, &dstAddr, cluster, ZCL_CMD_READ, FALSE, ZCL_FRAME_CLIENT_SERVER_DIR, FALSE, 0, 0, 2, (uint8 *)(data+5) );
	if (status != ZSuccess){
		serialSendAttributeResponseMsgError(dstAddr.addr.shortAddr, dstAddr.endPoint, cluster, BUILD_UINT16(data[5], data[6]), status);
	}
}