...
{
  osal_dispatch_entities = (uint8 *) osal_mem_alloc(TASK_CNT * 2);
  osal_memset(osal_dispatch_entities, OSAL_INVALID_DISPATCH_ID, TASK_CNT * 2);
...
void osal_enroll_dispatchid(uint8 taskid, ICall_EntityID dispatchid)
{
  ...
  osal_dispatch_entities[TASK_CNT + taskid] = dispatchid;
}
...
uint8 osal_msg_send( uint8 destination_task, uint8 *msg_ptr )
{
  ...
  if (destination_task & OSAL_PROXY_ID_FLAG)
  {
    ...
    {
      src = (ICall_EntityID) osal_dispatch_entities[taskid + TASK_CNT];
    }
